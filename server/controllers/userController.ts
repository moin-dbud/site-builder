...
async function generateProjectCodeInBackground(
    projectId: string,
    userId: string,
...
2. No markdown, no code fences, no explanations — HTML only, ready to render as-is.`
        ...
        console.error('[generateProjectCodeInBackground] Error:', error.code || error.message);
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: `An error occurred while generating your website: ${error.message}`,
                projectId
            }
        }).catch(() => {});
    }
...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Return immediate response with projectId so client navigates immediately (< 300ms)!
        res.json({ projectId: project.id, message: "Project created successfully" })

        // Trigger AI background website generation
        generateProjectCodeInBackground(project.id, userId, initial_prompt, creditsPerGeneration).catch(err => {
            console.error('[Background Generation Handler Error]:', err)
        })
